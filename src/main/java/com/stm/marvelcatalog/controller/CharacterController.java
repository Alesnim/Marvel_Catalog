...
public class CharacterController {
    ...
    public ResponseEntity<CharacterResponse> createCharacter(
        ...
        try {
            ...
            if (comics.length != 0) {
                dto.setComics(Stream.of(comics)
            ...
            comicsServiceImp.getAllComics().forEach(x -> {
                x.getCharacters().add(c);
                comicsServiceImp.updateComic(x);
            });
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "Return image"),
            @ApiResponse(code = 404, message = "Thumbnail not found"),
            @ApiResponse(code = 400, message = "Id invalid")
    })
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "Return list of comics"),
            @ApiResponse(code = 404, message = "Character not found"),
            @ApiResponse(code = 400, message = "Id invalid")
    })
    @GetMapping(value = "/character/{id}/comics", produces = MediaType.APPLICATION_JSON_VALUE)
    public Page<ComicResponse> getComicsCharacter(
        ...
        if (c.isEmpty()) {
            List<ComicResponse> comics = characterServiceImp.getCharacterById(id).getComics()
                    .stream()
